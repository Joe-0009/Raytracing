			...
			t_vec3 *rotation, t_vec3 *scale)
{
	...
	rotation->x = atan2(matrix.m[2][1] / scale->y, matrix.m[2][2] / scale->z);
	rotation->y = atan2(-matrix.m[2][0] / scale->x,
						sqrt(matrix.m[2][1] * matrix.m[2][1] +
							matrix.m[2][2] * matrix.m[2][2]) / scale->y);
	rotation->z = atan2(matrix.m[1][0] / scale->x, matrix.m[0][0] / scale->x);
}

/*
** Create view matrix for camera (look-at matrix)
*/
